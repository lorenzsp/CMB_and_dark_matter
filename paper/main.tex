...
All kinds of perturbations are proportional to the temperature fluctuations $\Theta$, so we  can associate the fluctuations to the displacement $x$ of spring-mass system. 
The fluctuations of the primordial fluid can be characteried by a typical scale $\lambda =2 \pi /k$ of oscillation/propagation that could depends on the local gravitational pull, pressure or properties. 
In addition, they are damped due to the expasion of the Universe and due to collisions, and ultimately they feel a constant gravitational pull from the overall fluid.
...
\end{equation}
We will discuss how with this simple model we can obtain some of the features of the CMB power spectrum at different scales.
...
We can compare the x displacement of the spring with the size of the oscillation $\lambda$
\[
\dfrac{x}{\lambda} \approx -\dfrac{g m}{\lambda k} \dfrac{\lambda}{\lambda} = -\dfrac{g \lambda}{\lambda^2 \omega_k ^2}
\]
Notice that the numerator is the gravitational potential of a uniform field and the denominator is proportional to the speed of the oscillations.\\
...
The temperature fluctuations are projected in the sky map and they appear to us as anisotropies in the sky map tempearature.
