...
The mass that elongates the spring and falls down is analogoues to the fluid falling and compressing inside a gravitational potential.\\
...
The poles are inversely proportional to a an angle on the sky, $l \sim 1/ \theta$, for small $l$ we are considering big scales of the sky map, viceversa for big $l$.
...
You might think that the rarefaction should be associated with the local minimum after the first peak, but this is not the case because the power spectrum is sensitive to the absolute value of the tempereature oscillations.
...
Since the first peak represents the first complete compression of the photon-bryon fluid, we can compare the size of the oscillations obtained with $k=0$ with the value obtained by multplying the sound speed of the fluid times the time of the collapse times the expansion factor.\\
...
Baryonic matter couples to radiation through the electromagnetic intereaction and it adds inertial mass to the oscillating photon-baryon fluid.
...
By eliminating gravitational potentials, photon-baryon acous- tic oscillations eliminate the alternating peak heights from baryon loading \cite{huCosmicMicrowaveBackground2001} as it is evident from Figure(\ref{DM_bary}b).\\
