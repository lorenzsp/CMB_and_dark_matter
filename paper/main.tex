...
Then, the physics of the hot plasma is explained with a simplified treatment in order to explain the main features of the power spectrum of the CMB.
...
However, the best way to measure the CMB spectrum is to use satellites.\\
The first satellite that was launched to observe the CMB was COBE. 
...
The energy density of radiation $u = \rho_r c^2 $ decreases with the power of four because photons experience not only the dilution due to the spatial expansion but their frequencies, accordingly their energies, decreased by the stretching of spacetime.\\
...
As a consequence of the Einstein's equivalence principle, photons that escape from a gravitational field are red-shifted.
If we consider the fluctuations of the CMB on large scales, the photons that tries to escape from the gravitational well created by matter are red-shifted according to the so-called Sachs-Wolfe effect. 
...
The mass that elongates the spring and falls down is analogues to the fluid falling and compressing inside a gravitational potential.\\
...
The position of the peaks is instead particularly sensitive to the curvature of the Universe, as show in Figure(\ref{DE_curv}a).
...
By multiplying this angle and the angular distance (specifically the angular distance) we obtain the size of the oscillations for Euclidean geometry $k =0$.
Since the first peak represents the first complete compression of the photon-baryon fluid, we can compare the size of the oscillations obtained with $k=0$ with the value obtained by multiplying the sound speed of the fluid times the time of the collapse times the expansion factor.\\
...
\end{center}
\caption{$\Omega_{tot} = 1-\Omega_{k0}$}
\label{DE_curv}
\end{figure}
...
What happens when you add mass to a spring and let it fall in the gravitational field of the Earth.  
